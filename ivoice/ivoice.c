 ...
 *  Author:   J. Zbiciak
 *  $Id: ivoice.c,v 1.10 2001/11/02 02:00:03 im14u2c Exp $
 * ============================================================================
...
uint_32 ivoice_tk(periph_t *per, uint_32 len)
{
    ...
    while (ivoice->sound_current < until)
    {
        ...
        while (ivoice->sc_tail < ivoice->sc_head)
        {
            ...
            ivoice->sample_frc += ivoice->rate * 358;
            ...
            while (ivoice->sample_frc > 3579545)
            {
                ivoice->sample_frc -= 3579545;
        ...
        /* ---------------------------------------------------------------- */
        samples = ((int)(until - ivoice->sound_current + 178)) / 179;
        ...
        ivoice->sound_current += did_samp * 179;
    }

abort:
    if ((ivoice->sound_current >> 1) < per->now) 
    {
        ivoice->snd_buf.drop++;
        return 0;
    }

    return (ivoice->sound_current >> 1) - per->now;
}
...
)
{
    ...
    ivoice->periph.min_tick  = 14934;
    ivoice->periph.max_tick  = 14934;
