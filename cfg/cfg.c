...
int cfg_setbind(cfg_t *cfg, const char *kbdhackfile)
{
    ...
    while (fgets(buf, 256, f) != NULL)
    {
        ...
        s2 = cmd;
        while (*s1 && !isspace(*s1) && s2 < &buf[255]) *s2++ = *s1++;
        if (s2 >= &buf[255]) bad = 1;
        *s2 = 0;
...
LOCAL const char *optchars= "E:e:g:s::f::F:?dhlqrP::x::z::a:w:B:C:M:m:"
                            "v::W::V::i::I::c:D:p:J:";
...
#define SER_REG(x,t,l,f)\
                            ...
                            &option_idx)) != EOF)
    {
        ...
        switch (c)
        {
            ...
            case 'z': cfg->disp_res   = value;                  break;
...
#define CHG_BIT(var, bit, to) (var) = ((var) & ~(bit)) | ((to) ? (bit) : 0)
...
#ifdef N900
    /* -------------------------------------------------------------------- */
    /*  On N900, simply force a few arguments to the only supported vals.   */
    /* -------------------------------------------------------------------- */
    //cfg->gfx_flags &= ~GFX_FULLSC;
    //cfg->gfx_flags &= ~GFX_DBLBUF;
    //cfg->disp_res = 5;
    ...
    {
        rx = res_x[cfg->disp_res];
        ry = res_y[cfg->disp_res];
        rd = res_d[cfg->disp_res];
    }
